...
export default function BriefCard({ title, description, meta, right, className = "", children }: BriefCardProps) {
  return (
    <div className={`bg-white rounded-[12px] border border-[#ececec] p-5 flex flex-col gap-3 ${className}`}>
      ...
      {description ? (
        <p className="text-sm leading-[18.62px] text-[#646464] min-h-[38px] overflow-hidden" style={{ display: '-webkit-box', WebkitLineClamp: 2, WebkitBoxOrient: 'vertical' }}>{description}</p>
      ) : null}
